...
public class IncomeActivity extends Fragment implements OnScrollListener {
    ListView mMainList;
    ...
    public static IncomeActivity newInstance(int position) {
        IncomeActivity frag = new IncomeActivity();
    ...
    public void mainListMake(String selectSortText)
    {
        ...
        if (mCategory.equals("전체"))
            queryString = "select * from familyevent where section = 1 order by " + selectSortText;
        else
            queryString = "select * from familyevent where category = '" + mCategory + "' and " +
                    "section = 1 order by " + selectSortText;
            ...
            Bundle savedInstanceState)
    {
        ...
        mMainList = (ListView) rootView.findViewById(R.id.incomeList);
