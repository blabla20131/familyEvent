...
public class MainListActivity extends Fragment implements OnScrollListener {
    ...
    public static MainListActivity newInstance(int position) {
        MainListActivity frag = new MainListActivity();
    ...
    public void mainListMake(String selectSortText)
    {
        ...
        if (mCategory.equals("전체"))
            queryString = "select * from familyevent where cast(date as integer) > " + nowDate + " order by " + selectSortText;
        else
            queryString = "select * from familyevent where category = '" + mCategory + "' and " +
                    "cast(date as integer) > " + nowDate + " order by " + selectSortText;
            ...
            Bundle savedInstanceState)
    {
        ...
        View rootView = inflater.inflate(R.layout.activity_main_list, container, false);
        mMainList = (ListView) rootView.findViewById(R.id.mainList1);
